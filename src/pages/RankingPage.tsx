...
import { Trophy, TrendingDown, TrendingUp, ChevronLeft, ChevronRight, Calendar } from 'lucide-react';
...
export default function RankingPage() {
  ...
  const { data: ranking = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Fetch transactions for all profiles this month
      ...
      // Fetch extra income for all profiles this month
      ...
      const result: ProfileRanking[] = profiles.map((p) => {
        ...
        return {
          ...
          totalDespesas,
          totalRenda,
          economia: totalRenda - totalDespesas,
        };
      });

      // Sort: highest economia first
        ...
        <Card className="card-glass animate-fade-up">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground">
              Crie pelo menos <strong>2 perfis financeiros</strong> para ver o ranking.
            </p>
          </CardContent>
        </Card>
      ) : (
          ...
          {ranking.map((profile, index) => {
            ...
            return (
              <Card
                key={profile.id}
                className={cn(
                  'card-glass transition-all hover:scale-[1.01]',
                ...
                <CardContent className="py-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col items-center min-w-[40px]">
                      {index < 3 ? (
                        <Trophy className={cn('mb-0.5', medalColors[index])} size={22} />
                      ) : (
                    ...
                    <div
                      ...
                      style={{ backgroundColor: profile.color + '22', borderColor: profile.color, borderWidth: 2 }}
                    >
                      {profile.icon}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex justify-between mt-1.5 text-xs text-muted-foreground">
                        <span className="flex items-center gap-1">
                          ...
                          R$ {profile.totalRenda.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                        </span>
                        <span className="flex items-center gap-1">
                          <TrendingDown size={12} className="text-destructive" />
                          R$ {profile.totalDespesas.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                ...
                <p className="text-sm font-bold text-emerald-500">
                  R$ {ranking.reduce((s, r) => s + r.totalRenda, 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                </p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Total Despesas</p>
                <p className="text-sm font-bold text-destructive">
                  R$ {ranking.reduce((s, r) => s + r.totalDespesas, 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                </p>
              </div>
              <div>
                ...
                <p className={cn(
                  'text-sm font-bold',
                  ranking.reduce((s, r) => s + r.economia, 0) >= 0 ? 'text-emerald-500' : 'text-destructive'
                )}>
